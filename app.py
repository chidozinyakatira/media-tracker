...
def get_sheets():
    ...
    return {
        "books":  get_or_create("Books",  BOOK_COLS),
        "series": get_or_create("Series", SERIES_COLS),
    }
...
with book_tab:
    ...
    rated     = [b for b in books if b.get("rating") and b["rating"] != "Not rated"]
    avg_b     = f"{sum(int(r['rating'][0]) for r in rated)/len(rated):.1f} ★" if rated else "—"
    ...
    with btab2:
        ...
        c1, c2 = st.columns(2)
        with c1:
            title  = st.text_input("Title *",  value=ex.get("title",""))
            author = st.text_input("Author *", value=ex.get("author",""))
        ...
        with c2:
            ...
            date_f = st.date_input("Date finished", value=d_val)
            reread = st.checkbox("Would re-read", value=str(ex.get("reread","")).lower() in ["true","yes","1"])
            cover  = st.text_input("Cover image URL (optional)", value=ex.get("cover_url",""))

        review = st.text_area("Review",  value=ex.get("review",""), height=80)
        notes  = st.text_area("Notes",   value=ex.get("notes",""),  height=60)
...
with series_tab:
    ...
    rated_s   = [s for s in series if s.get("rating") and s["rating"] != "Not rated"]
    avg_s     = f"{sum(int(r['rating'][0]) for r in rated_s)/len(rated_s):.1f} ★" if rated_s else "—"
    ...
    with stab2:
        ...
        with c1:
            title   = st.text_input("Title *",   value=ex.get("title",""))
            creator = st.text_input("Creator / Showrunner", value=ex.get("creator",""))
        ...
        with c2:
            ...
            seasons  = st.number_input("Seasons watched", min_value=0, max_value=50,
                                       value=int(ex.get("seasons",0) or 0))
            raw_d    = ex.get("date_finished","")
            d_val    = date.fromisoformat(str(raw_d)[:10]) if raw_d else date.today()
        ...
        poster = st.text_input("Poster image URL (optional)", value=ex.get("poster_url",""))
        review = st.text_area("Review", value=ex.get("review",""), height=80)
        notes  = st.text_area("Notes",  value=ex.get("notes",""),  height=60)
