            ...
            for status in BOOK_STATUS:
                ...
                for b in group:
                    ...
                    if orig_idx is not None:
                        ...
                        with ca:
                            if st.button("✏️ Edit", key=f"eb_{status}_{orig_idx}_{b.get('title','')[:5]}"):
                                st.session_state.edit_book = orig_idx
                                st.rerun()
                        with cb:
                            if st.button("🗑️ Delete", key=f"db_{status}_{orig_idx}_{b.get('title','')[:5]}"):
            ...
            for status in SERIES_STATUS:
                ...
                for s in group:
                    ...
                    if orig_idx is not None:
                        ca, cb = st.columns([1,1])
                        with ca:
                            if st.button("✏️ Edit", key=f"es_{status}_{orig_idx}_{s.get('title','')[:5]}"):
                                st.session_state.edit_series = orig_idx
                                st.rerun()
                        with cb:
                            if st.button("🗑️ Delete", key=f"ds_{status}_{orig_idx}_{s.get('title','')[:5]}"):
