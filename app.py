            ...
            for status in BOOK_STATUS:
                ...
                for _bi, b in enumerate(group):
                    ...
                    if orig_idx is not None:
                        ...
                        with ca:
                            if st.button("✏️ Edit", key=f"eb_{status}_{_bi}"):
                                st.session_state.edit_book = orig_idx
                                st.rerun()
                        with cb:
                            if st.button("🗑️ Delete", key=f"db_{status}_{_bi}"):
            ...
            for status in SERIES_STATUS:
                ...
                for _si, s in enumerate(group):
                    ...
                    if orig_idx is not None:
                        ca, cb = st.columns([1,1])
                        with ca:
                            if st.button("✏️ Edit", key=f"es_{status}_{_si}"):
                                st.session_state.edit_series = orig_idx
                                st.rerun()
                        with cb:
                            if st.button("🗑️ Delete", key=f"ds_{status}_{_si}"):
